...
export type ImmersiveAppConfig = { identity: ProductIdentity; navigation: readonly NavigationItem[]; experts: readonly WelcomeExpert[]; welcome: { greeting: string; expertIds: readonly string[]; recommendations: readonly WelcomeRecommendation[]; expertRecommendations: Readonly<Record<string, readonly string[]>> }; /** Whether this product exposes connector selection in Composer. */ showConnectorSelect?: boolean; /** Optional Composer expert menu labels. Defaults to the shared Agent list. */ composerExpertOptions?: readonly string[]; /** Visual keys for Composer expert options, keyed by expert label. */ composerExpertVisualKeys?: Readonly<Record<string, WelcomeExpert['visualKey']>>; renderProductBlock?: ImmersiveProductBlockRenderer }

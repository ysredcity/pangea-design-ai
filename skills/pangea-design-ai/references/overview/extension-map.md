...
| Copilot 产物展示与产品块 action 回写 | `skills/agent-ux-react/templates/copilot-starter/src/pages/ContractReview.tsx` 的 `artifact` state、`routeArtifact={setArtifact}` 与 `onProductBlockAction`；传给 `packages/agent-ui/src/copilot/copilot-app.tsx` 的 `workspace` | 沉浸式 `ArtifactPanel`、右侧产物 Tab；把 action 结果写入 shared panel/canvas |
...
| 产品身份、导航、欢迎专家/推荐 | `skills/agent-ux-react/templates/immersive-starter/src/components/agent-layout/app-config.ts` 的 `AppConfig` | 用 AppConfig 代替业务场景、面板容器或产品块扩展 |

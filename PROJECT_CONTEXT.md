...
├── releases/                           # 可分发的版本归档（历史版本保留，不覆盖、不改名复用）
│   ├── agent-ux-react-v0.1.0.zip         # 更名前的历史归档
│   ├── pangea-design-ai-v0.1.0.zip
│   └── pangea-design-ai-v0.2.0.zip       # 当前基线（不含 node_modules/dist/.workbuddy）
...
- **2026-09-03 — 修复产物形态错位（只做方案 A，website 保留）**：用户反馈生成产物像 website 文档页。查明根因**不在 `website/`**，而在 skill 指令层：① `project-structure.md` 通篇维护者视角（根级路径与 `npm run sync:agent-ui`、根 gate），把消费方 agent 的工作目录锚回本仓库，于是它照抄根目录的 `website/`；② 两阶段门只规定第一轮交付需求文档，没说明文档不落盘、也没规定阶段二产物必须是可运行应用；③ quality-gates 反面清单未覆盖形态错位。已做：SKILL.md 新增「第三道门：交付物只能是可运行的智能体应用」（含 5 类错误形态表 + 首屏可发起委托的自检），删除 SKILL.md 中唯一的 Website/Showcase 提及（对消费方只有污染价值）；`requirement-intake.md` 写明需求文档只在对话里给出；`quality-gates.md` 新增 G0.5 硬门禁并给反面清单补 3 条（形态错位、模仿仓库文档站结构、在 skill 仓库里改代码充当交付）；`project-structure.md` 重写为消费者视角优先 + 「仅维护者：物化分发管线」独立小节。**顺带修一个真实地雷**：`build-catalog.mjs` 仍扫描 Phase 3 已移除的 `references/component-selection/`，按 SKILL.md「改元数据后需重跑生成器」照做会把 `catalog.json` 从 40 个组件清空为 0（本轮误跑触发过，已 `git checkout` 还原后修好生成器，改为递归扫描 `references/components/`，结果 2 shells + 40 components、doc 路径全部有效）。验证：`npm run gate`、`check:component-docs`、`check:agent-ui-drift`、文档内链接目标存在性自检、`git diff --check` 均通过。**方案 B（删除 `website/`）用户决定暂不执行**，`website/` 与根 gate 链保持原样。
- **2026-09-03 — 审批链路修复 + 校验器接回 + SKILL 路由表（外部试用反馈）**：用户用另一 agent 试用后反馈"产出可以但过程非常长"，并附了一份 5 条问题清单。核实后**修正两处结论**：① 清单第 4 条「trigger 宽泛词/导出顺序」在沉浸式**前提不成立**——immersive 按会话 id 取场景（`scenesById`），新输入一律走 `createDraftScene`，`matchTrigger` 从未被沉浸式调用，`conversation-data.ts` 与 `scenes.ts` 里 `trigger` 出现 0 次；真实缺口是**沉浸式没有"用户自然语言进入已写场景"的路径**（那位 agent 自己补了匹配才撞上顺序问题）。② 清单第 5 条比报告更严重：`check-scripts.mjs` 对沉浸式**整体短路**（`if (existsSync(scenes.ts)) continue`），第 74-90 行那套审批校验一条都没执行，审批契约校验实际是死代码。另外发现清单未提的隐藏耦合：`conversation-flow.tsx` 要求 `turn.productBlock && turn.approvalOutcomes` 才渲染续流程，而审批完成后原确认卡被传 `undefined`，于是决策一落地结构化信息全部消失——这才是"确认卡不显示"与"缺 productBlock"两条症状的共同根因。已完成 P0/P1/P2：契约加 `productBlock`（改了 3 处：immersive contracts、script-engine types、模板本地 `conversation-data.ts` 同名类型）；`ApprovalContinuation` 透传渲染并把续流程判定改为 `awaitingApproval`；`approvalStatus` 改为 `deriveApprovalStatus` 从末轮派生（覆盖 initial 与 `onStartConversation` 两条路径），移除 `sidebar.tsx` 与模板 `App.tsx` 两处手写 `pending`；`check-scripts.mjs` 用括号深度扫描器（跳过字符串/注释，避免 `[[file:…]]` 干扰）校验 TS 场景，审批规则改双向（末轮、配 confirm-card、双结果、孤立 outcomes 报错），新增 trigger 重复/子串冲突机检；SKILL.md 新增 9 行任务→最小必读集路由表。**校验器做过 4 组负面测试**（非末轮、缺 rejected、孤立 outcomes、缺 confirm-card、trigger 重复+子串）确认能抓出违规，再确认现有数据干净（沉浸式 7 组 turns 通过）。验证：`npm run gate`、drift、component-docs、`git diff --check` 全通过。**遗留**：沉浸式的自然语言场景进入路径仍未提供（属功能新增，未在本轮做）；bundle >500 kB 提示仍在。
- **2026-09-03 — 补上沉浸式自然语言场景进入路径（上一条的遗留项）**：`ImmersiveConversationScene` 新增 `trigger`；`agent-shell.tsx` 新增 `resolveSceneForInput()`，新对话首句先 `matchTrigger` 命中已写场景，未命中才回退 `createDraftScene`。命中时把首轮 user 消息替换为用户实际输入与真实附件、并给场景换新 id（避免与侧栏 scene id 冲突），后续轮保持剧本。**顺带从根上消掉那类顺序 bug**：`matchTrigger` 的 keyword 由「声明顺序首个命中」改为**最长命中优先**（长度相同才回退声明顺序），结果不再依赖 `scenes` 导出顺序；regex 仅在 keyword 全未命中时兜底；签名放宽为结构化泛型 `TScene extends { id, trigger? }`，沉浸式与 JSON 剧本共用同一实现，不复制逻辑。已用仓库自带 tsc 转译真实 `match.ts` 做行为验证：`[broad, exact]` 与 `[exact, broad]` 两种顺序都命中 exact，只有宽泛词匹配时仍返回 broad，无匹配返回 null。六个示例场景配上 trigger（专属动词短语，无跨场景重复/子串），其中「调研瑞幸」「本周工作进展／生成周报」对应现有首屏推荐，点推荐即进完整剧本；`app-config.ts` 加注释说明推荐语与 trigger 的关系。机检扩展：`validateTsTriggers` 让 trigger 冲突规则也覆盖 TS 场景（此前只覆盖 JSON），已负面测试验证能抓出子串冲突。文档：`conversation-contracts.md` 原先那句"沉浸式不使用 trigger"已作废，改写为「场景的两条进入路径」表 + 最长命中优先的两条硬约束；`extension-map.md` 新增"新增可被用户说出来触达的场景"行。**同步脚本新增一条重写规则** `../../script-engine` → `@/agent-ui/script-engine`，否则物化后模板会解析到不存在的路径。验证：`npm run gate`、drift、component-docs、`git diff --check` 全通过。仍未做：bundle >500 kB 拆包。
- **2026-09-06 — 发布 v0.2.0（更新日志整理 + 打包）**：日期已用 `date` 核实为 2026-09-06（注入上下文里的 09-03 已过期，勿信）。按语义化版本定 **0.2.0**（minor：有新功能 trigger 自然语言入口 + 行为变更 matchTrigger 语义 + 一批修复）。CHANGELOG 把此前散落的 Unreleased 内容（含两个重复的 `### Added` 段）归并为 `## [0.2.0] - 2026-09-06`，分 Added / Changed / Fixed，并补上 skill 更名条目与升级方式说明（重装新 ZIP 并删除旧 `agent-ux-react/` 目录；`@agent-ux/agent-ui` 包名与 imports 不变，已生成的工程无需改动）；Unreleased 只留"后续"，新增 bundle 拆包一项。顺手修掉 CHANGELOG 里 trigger 机检那条已过时的表述（还写着"声明顺序首个命中"）。版本号同步到 5 个 manifest（根、agent-ui、website、两套模板）并用 `npm install --package-lock-only` 重算 lockfile；SKILL.md 基线段、README、CONTRIBUTING 均已更新，CONTRIBUTING 新增发布产物命名与排除规则约定。**打包产物** `releases/pangea-design-ai-v0.2.0.zip`（596,964 bytes，285 个文件），已验证：`unzip -t` 完整性、入口 `pangea-design-ai/SKILL.md` 存在、无 `node_modules`/`dist`/`.workbuddy`，并逐项确认本轮改动确实入包（SKILL.md 基线为 v0.2.0、模板 version 0.2.0、6 处 trigger、deriveApprovalStatus/resolveSceneForInput、validateTsScenes/validateTsTriggers、按任务读取路由表）。打包前 `npm run gate` 退出码 0。历史归档全部保留。未 commit/push。